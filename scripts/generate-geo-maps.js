...
// Branches data copied directly from src/data/branches.ts for script environment
const tckBranches = [
...
function generateKML() {
  ...
  for (const branch of tckBranches) {
    const slugifiedDistrict = branch.district.toLowerCase()
      .replace(/ğ/g, 'g')
      .replace(/ü/g, 'u')
      .replace(/ş/g, 's')
      .replace(/ı/g, 'i')
      .replace(/ö/g, 'o')
      .replace(/ç/g, 'c');
    const branchUrl = `https://www.tckilaclama.com/hizmet/istanbul-${slugifiedDistrict}-bocek-ilaclama`;

    placemarks += `    <Placemark>
      <name>${branch.name}</name>
      <description>TCK İlaçlama İstanbul ${branch.district} Bölge Operasyon ve Hizmet Merkezi. Detaylı bilgi için: ${branchUrl}</description>
      <Point>
        <coordinates>${branch.lng},${branch.lat},0</coordinates>
      </Point>
    </Placemark>\n`;
  }
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    <name>TCK Ilaclama Istanbul Harita Pinleri</name>
    <description>TCK Ilaclama 28 Aktif Bolge Konumu ve Koordinatlari</description>
\n${placemarks}  </Document>
</kml>`;

  const destPath = path.join(__dirname, '..', 'public', 'locations.kml');
  fs.writeFileSync(destPath, kmlContent, 'utf8');
  console.log(`✅ locations.kml generated successfully at: ${destPath}`);
}

function generateGeoSitemap() {
  const geoSitemapContent = `<?xml version="1.0" encoding="UTF-8"?>
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
        xmlns:geo="http://www.google.com/geo/schemas/sitemap/1.0">
  <url>
    <loc>https://www.tckilaclama.com/locations.kml</loc>
    <geo:geo>
  ...
  const destPath = path.join(__dirname, '..', 'public', 'geo-sitemap.xml');
  fs.writeFileSync(destPath, geoSitemapContent, 'utf8');
  console.log(`✅ geo-sitemap.xml generated successfully at: ${destPath}`);
}
