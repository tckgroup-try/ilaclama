...
// Target pest specifications and their dedicated high-quality Unsplash images
const pestSpecs = {
  bocek: {
    name: 'Böcek',
...
function generate() {
  ...
  // Generate interlinked Tag Cloud links
  const districtLinks = districtsArr.map((d, index) => {
    const pestSlug = pestOrder[index % pestOrder.length];
    const pestSpec = pestSpecs[pestSlug];
    return `    <a href="https://istanbul-bocek-ilaclama.readme.io/docs/istanbul-${d.districtSlug}-${pestSlug}-ilaclama-rehberi" style="background: #fff; border: 1px solid #cbd5e1; padding: 6px 12px; border-radius: 20px; font-size: 13px; color: #475569; text-decoration: none; font-weight: 500; display: inline-block;">${d.districtName} ${pestSpec.name} İlaçlama</a>`;
  }).join('\n');
  ...
  districtsArr.forEach((dist, idx) => {
    ...
    // Grab first few neighborhoods for embedding into paragraph text
    ...
    // Dynamically build personalized tag clouds based on district and pest slug
    const customTags = `
    <a href="https://www.tckilaclama.com/hizmet/istanbul-${dist.districtSlug}-${pestSlug}-ilaclama" style="font-size: 15px; color: #0284c7; text-decoration: none; font-weight: bold; margin: 0 5px;">#${dist.districtSlug} ${pestSlug} ilaçlama</a>
    <a href="https://www.tckilaclama.com/hizmet/istanbul-${dist.districtSlug}-bocek-ilaclama" style="font-size: 12px; color: #475569; text-decoration: none; margin: 0 5px;">#${dist.districtSlug} dezenfeksiyon firması</a>
    <a href="https://www.tckilaclama.com" style="font-size: 14px; color: #e11d48; text-decoration: none; font-weight: bold; margin: 0 5px;">#garantili ${pest.name.toLowerCase()} kontrolü</a>
    <a href="https://wa.me/905016355053" style="font-size: 13px; color: #0d9488; text-decoration: none; margin: 0 5px;">#acil ${dist.districtSlug} ilaçlama</a>
    <a href="https://www.tckilaclama.com/subelerimiz" style="font-size: 12px; color: #4f46e5; text-decoration: none; font-weight: bold; margin: 0 5px;">#${dist.districtSlug} nöbetçi ilaçlama</a>
    <a href="https://www.tckilaclama.com/hizmet/${pestSlug}-ilaclama" style="font-size: 16px; color: #b45309; text-decoration: none; font-weight: bold; margin: 0 5px;">#en iyi ${pest.name.toLowerCase()} ilacı</a>
    `;
...
`;
  ...
  console.log(`\n🎉 Successfully generated 39 district parasite files with fully customized tag clouds!`);
}
